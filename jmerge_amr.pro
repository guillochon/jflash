...
case params.corners of
  ...
  0: begin   ; no corners
    dx_fine = (max(tree[*].bndBox[1,0]) - $
               min(tree[*].bndBox[0,0]))/ $
      (params.ntopx*params.nxb*2L^(lwant-1))
    ...
    dy_fine = (max(tree[*].bndBox[1,1]) - $
               min(tree[*].bndBox[0,1]))/ $
      (params.ntopy*params.nyb*2L^(lwant-1))
    ...
    if (params.ndim EQ 3) then begin
      dz_fine = (max(tree[*].bndBox[1,2]) - $
                 min(tree[*].bndBox[0,2]))/ $
        (params.ntopz*params.nzb*2L^(lwant-1))
  ...
  1: begin  ; corners
    dx_fine = (max(tree[*].bndBox[1,0]) - $
               min(tree[*].bndBox[0,0]))/ $
      (params.ntopx*nxb_centered*2L^(lwant-1))
    ...
    dy_fine = (max(tree[*].bndBox[1,1]) - $
               min(tree[*].bndBox[0,1]))/ $
      (params.ntopy*nyb_centered*2L^(lwant-1))
    ...
    if (params.ndim EQ 3) then begin
      dz_fine = (max(tree[*].bndBox[1,2]) - $
                 min(tree[*].bndBox[0,2]))/ $
        (params.ntopz*nzb_centered*2L^(lwant-1))
...
case params.corners of
  ...
  0: begin  ; no corners
    nx_domain = params.ntopx*params.nxb*2L^(lwant-1)
    ny_domain = params.ntopy*params.nyb*2L^(lwant-1)
    if params.ndim EQ 3 then nz_domain = $
      params.ntopz*params.nzb*2L^(lwant-1)
  end

  1: begin  ; corners
    nx_domain = params.ntopx*nxb_centered*2L^(lwant-1)
    ny_domain = params.ntopy*nyb_centered*2L^(lwant-1)
    if params.ndim EQ 3 then nz_domain = $
      params.ntopz*nzb_centered*2L^(lwant-1)
  end
...
; from rounding
;------------------------------------------------------------------------------
xrange_min_index = (where(xrange[0] LE x, /L64))[0]
xrange_max_index = xrange_min_index + nx - 1

yrange_min_index = (where(yrange[0] LE y, /L64))[0]
yrange_max_index = yrange_min_index + ny - 1

if (params.ndim EQ 3) then begin
  zrange_min_index = (where(zrange[0] LE z, /L64))[0]
...
for i = 0l, numPlotBlocks-1 do begin
  ...
  xindex = (where(tree[curBlk].bndBox[0,0] LE x, /L64))[0] - xrange_min_index
  yindex = (where(tree[curBlk].bndBox[0,1] LE y, /L64))[0] - yrange_min_index
  if (params.ndim EQ 3) then $
    zindex = (where(tree[curBlk].bndBox[0,2] LE z, /L64))[0] - zrange_min_index
