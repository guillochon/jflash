...
for f=rng[0],rng[1],rng[2] do begin
	...
	for n=0L, (numblocks-1) do begin
		...
		if (~keyword_set(include_selfbound)) then begin
			gpot = unk[gvar,n,*,*,*]
			gpot = congrid(reform(gpot), nc, nc, nc, /center, cubic=-0.5)

			avelx = unk[vxvar,n,*,*,*] - peakvec[3]
			avely = unk[vyvar,n,*,*,*] - peakvec[4]
			avelz = unk[vzvar,n,*,*,*] - peakvec[5]
			avelx = congrid(reform(avelx), nc, nc, nc, /center, cubic=-0.5)
			avely = congrid(reform(avely), nc, nc, nc, /center, cubic=-0.5)
			avelz = congrid(reform(avelz), nc, nc, nc, /center, cubic=-0.5)
		...
		dens = congrid(reform(unk[dvar,n,*,*,*]), nc, nc, nc, /center, cubic=-0.5)
		...
		hvelx = congrid(reform(unk[vxvar,n,*,*,*]), nc, nc, nc, /center, cubic=-0.5)
		hvely = congrid(reform(unk[vyvar,n,*,*,*]), nc, nc, nc, /center, cubic=-0.5)
		hvelz = congrid(reform(unk[vzvar,n,*,*,*]), nc, nc, nc, /center, cubic=-0.5)
		hvelx = hvelx - ptvec[3]
		hvely = hvely - ptvec[4]	
		hvelz = hvelz - ptvec[5]	
		temp = -0.5*(hvelx^2 + hvely^2 + hvelz^2) + gptpot
