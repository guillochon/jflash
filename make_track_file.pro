; make_track_file.pro
; Processes orbit.dat files generated by MultiTidal for use as input for other jflash routines.
;
...
; PARAMETERS
; **Required**
; filename (str)                           - Name of the orbit info file (Usually 'orbit.dat')
;
; **Optional**
; offset (int)                             - Number of columns to offset the input columns by. Useful for older versions of MT.
; outfile (str)                            - Name of output file (Default: 'track.dat', except for mode 3).
; mode (int)                               - Determines type of output file. Options are:
; 										     mode 1: Follow center of mass.
; 										     mode 2: Follow point mass.
; 										     mode 3: Return full orbit.dat, but with restarts removed.
; 										             Used for certain derived variables such as 'bhbound' and 'selfbound'.
; 										     mode 4: Region list for VAPOR.
; stride (int)                             - Only include every nth row in output.
; nsmooth (int)                            - Smooth the data over this many rows.
; include_pt (bool)                        - Include point mass information in output.
; region ([[dbl,dbl],[dbl,dbl],[dbl,dbl]]) - Region boundaries for mode 4.
; plotcadence (dbl)                        - Time between rows for output in mode 4.
; plotnums ([int,int])                     - Range of frame numbers to include in mode 4.
