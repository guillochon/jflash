...
FUNCTION jProject_Vol, vol, x_sampleIn, y_sampleIn, z_sampleIn, $
...
zf_sample = Double(z_sample)
zf_sample_m1 = zf_sample - 1.0
z_sample_m1 = z_sample - 1L
z_max = Double(z_sample - 1L)
...
depth_q = (N_ELEMENTS(depth_qIn) GT 0) ? 0.0 > Double(depth_qIn[0]) < 1.0 : 1.0
depth_q = 1.0 - depth_q
...
IF (N_Elements(transIn) GT 0) THEN BEGIN
   IF (N_Elements(transIn) NE 16) THEN BEGIN
      ...
      RETURN, Bytarr(xsize, ysize)
   ENDIF ELSE BEGIN
      trans = Double(Reform(transIn, 4, 4))
   ENDELSE
ENDIF ELSE BEGIN
   trans = !P.T
ENDELSE
...
x_ind = (((Double(size_vol[1]) * Findgen(x_sample) / Double(x_sample - 1L)) * $
   !X.S[1]) + !X.S[0]) # Replicate(1.0, y_sample)
y_ind = Replicate(1.0, x_sample) # $
        (((Double(size_vol[2]) * Findgen(y_sample) / Double(y_sample - 1L)) * $
   !Y.S[1]) + !Y.S[0])

max_vol = Double(Max(vol))
...
IF Keyword_Set(z_buffer) THEN BEGIN ; *** Use Z-Buffer info.
   ...
   FOR k=0L, z_sample_m1 DO BEGIN
      ...
      indx = Double(size_vol[1]) * index[*, 0] / index[*, 3]
      indy = Double(size_vol[2]) * index[*, 1] / index[*, 3]
      indz = Double(size_vol[3]) * index[*, 2] / index[*, 3]
...
ENDIF ELSE BEGIN ; *** Don't use Z-Buffer.
   ...
   ELSE BEGIN
      CASE vol_type OF
         ...
         4: img = Fltarr(x_sample, y_sample)
         5: img = Dblarr(x_sample, y_sample)
